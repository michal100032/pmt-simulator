...
#define SPI_CLK_FREQ 5000000 // 5 MHz SPI clock

#include "AD536x.h"

// constructor...
AD536x::AD536x(SPIClass& spi, int clr, int ldac, int reset) 
	: _spi(spi), _clr(clr), _ldac(ldac), _reset(reset)
{		
...
void AD536x::writeCommand(unsigned long cmd){
	_spi.beginTransaction(SPISettings(SPI_CLK_FREQ, MSBFIRST, SPI_MODE1));	
	_spi.transfer((cmd >> 16) & 0xFF);
	_spi.transfer((cmd >> 8) & 0xFF);
	_spi.transfer(cmd & 0xFF);
	_spi.endTransaction();
}
